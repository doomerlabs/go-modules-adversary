#!/usr/bin/env node
import { createRequire as __goModulesCreateRequire } from 'node:module'; const require = __goModulesCreateRequire(import.meta.url);
...
var domain = {
  ...
  rules: [
    ...
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} same-module replacement${count === 1 ? "" : "s"} override every selected version of a required module.`,
...
function tidyOrphanRequireSignals(files) {
...
function requireDocumentsNonGoConsumer(source, line) {
  const text2 = requireContext(source, line).toLowerCase();
  return /\b(?:non-go|hugo|theme|makefile|external tool|code generator|protoc)\b/.test(text2);
}
function moduleDirectory(path) {
  if (path === "go.mod") return ".";
  return path.replace(/\/go\.mod$/, "");
}
function owningModuleDir(path, modDirs) {
...
function moduleIsImported(module2, imported) {
  for (const path of imported) {
    if (path === module2 || path.startsWith(`${module2}/`)) return true;
  }
  return false;
}
function requireDocumentsTidyException(source, line) {
  const text2 = requireContext(source, line).toLowerCase();
  return /(?:do not|don't|must not|never)\s+(?:run\s+)?go mod tidy|hugo mod/.test(text2);
}
...
function sameModuleReplaceSignals(file) {
...
function replacementLines(source) {
  const results = [];
...
function parseReplacementLine(line) {
  const withoutComment = line.replace(/\s*\/\/.*$/, "").trim();
  if (withoutComment === "" || withoutComment.startsWith("//")) return void 0;
  ...
  if (oldFields.length < 1 || oldFields.length > 2 || newFields.length < 1 || newFields.length > 2) return void 0;
  return {
...
var GO_MODULES_MODEL_PROMPT = `You are reviewing Go module metadata for reproducibility, upgrade safety, and ownership.

Authority:
- replace directives (local and mutable branches)
- excludes that hide upgrade constraints
- missing or inconsistent go.sum / workspace sums
- dependency ownership and retract risks visible in the change

Do NOT review application Go source style or runtime security.
...
function buildModelReviewRequestFromDiscovery(change, analysis, files) {
  ...
  return {
    ...
    evidenceById,
    request: {
      prompt: GO_MODULES_MODEL_PROMPT,
      input,
      schema: GO_MODULES_MODEL_SCHEMA,
      budget: {
...
async function applyModelModulesReview(ctx, output, evidenceById, staticSeverities = [], staticPrimaryConcern) {
  ...
  for (const observation of output.observations.slice(0, MAX_MODEL_OBSERVATIONS)) {
    ...
    ctx.review.observe({
      key: `go-modules.model.${observation.id}`,
...
async function runModelModulesReview(ctx, analysis, files, staticSeverities = [], staticPrimaryConcern) {
...
async function reviewDomain(ctx, analysis, discoveryFiles = []) {
  ...
  const concern = primary.rule.concern;
  if (concern) {
    ctx.review.opinion(
      formatOpinion({
    ...
    ctx.review.opinion({
      ship: primary.rule.severity === "low",
      summary: primary.rule.severity === "low" ? `I would merge this change and address ${primary.rule.title.toLowerCase()} as follow-up.` : `I would address ${primary.rule.title.toLowerCase()} before merging.`
    });
  }
}
